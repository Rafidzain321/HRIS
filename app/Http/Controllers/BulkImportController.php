...
class BulkImportController extends Controller
{
    ...
    public function importCcpm(Request $request)
    {
        ...
        if ($results['imported'] > 0) {
            ActivityLog::record('import', 'CCPM', 'Import Excel', "Import CCPM: {$results['imported']} berhasil, {$results['skipped']} di-skip, " . count($results['errors']) . ' error');
        }
    ...
    public function importDriver(Request $request)
    {
        ...
        if ($results['imported'] > 0) {
            ActivityLog::record('import', 'Driver', 'Import Excel', "Import Driver: {$results['imported']} berhasil, {$results['skipped']} di-skip, " . count($results['errors']) . ' error');
        }
        return redirect()->back()->with('import_result', $results);
    }
    ...
    public function importTraining(Request $request)
    {
        ...
        if ($results['imported'] > 0) {
            ActivityLog::record('import', 'Training', 'Import Excel', "Import Training: {$results['imported']} berhasil, {$results['skipped']} di-skip, " . count($results['errors']) . ' error');
        }
        return redirect()->back()->with('import_result', $results);
    }
